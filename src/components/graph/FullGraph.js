...
function FullGraph({
  ...
  const getCustomGraph = () => {
    if (type.includes("bar"))
      ...
      return setChartView(
        " ",
        <CountryProvinceTable countryName="" defaultData={data} />,
        true
      );
  };
  const setChartView = (caseType, child, globalChiled = false) => {
    if (globalChiled) return child;
    else
      return (
        <Card
          key={caseType}
